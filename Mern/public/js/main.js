            ...
            fetch('/search', {
                ...
                .then(data => {
                    ...
                    data.products.forEach(product => {
                        ...
                        btn.style.backgroundColor = '#fd5c63';

                        btn.addEventListener('click', async function (event) {
                            event.preventDefault();
                            const customerId = localStorage.getItem("userIdForUse");

                            // Create a new object with quantity set to 1
                            const new_product = { ...product, quantity: 1 };

                            // Call addToCart with customerId and productWithQuantity
                            await addToCart(customerId, new_product);
                        });
...
function deleteCartItem(productId) {
    ...
    fetch('/delete_from_cart', {
        ...
        body: JSON.stringify({ userId: customerId, productId: productId }) // Send both customerId and productId in the request body
    }).then(response => {
        if (!response.ok) {
